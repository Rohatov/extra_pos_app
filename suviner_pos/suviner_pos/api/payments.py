...
from suviner_pos.suviner_pos.api.utilities import ensure_child_doctype


def get_suviner_pos_credit_redeem_remark(invoice_name):
    return _("Suviner POS credit redemption for Sales Invoice {0}").format(invoice_name)
    ...
    if data.get("redeemed_customer_credit"):
        ...
        for row in data.get("customer_credit_dict"):
            if row["type"] == "Invoice" and row["credit_to_redeem"]:
                ...
                jv_doc.user_remark = get_suviner_pos_credit_redeem_remark(invoice_doc.name)
                jv_doc.set_missing_values()
                try:
                    jv_doc.save()
                    jv_doc.submit()
                except Exception as e:
                    frappe.log_error(frappe.get_traceback(), "Suviner POS JV Error")
