...
from suviner_pos.suviner_pos.doctype.pos_coupon.pos_coupon import check_coupon_code
...
def _get_promotional_scheme_offers(pos_profile):
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "Suviner POS - Failed to fetch Promotional Schemes")
        return []
    ...
    for row in promotional_schemes:
        try:
            ...
            frappe.log_error(
                ...
                f"Suviner POS - Unable to load Promotional Scheme {row.name}",
            )
            continue
