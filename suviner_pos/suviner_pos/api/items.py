...
def get_items_details(pos_profile, items_data, price_list=None, customer=None, strict_price_list=None):
    ...
    `strict_price_list` — ESKI parametr, endi eʼtiborga olinmaydi: narx
    HAR DOIM faqat soʻralgan price listdan olinadi (topilmasa 0).
    """
...
def get_item_detail(item, doc=None, warehouse=None, price_list=None, company=None):
    ...
    # DIQQAT: ustun yo'q saytda (custom field sinxron bo'lmagan bo'lsa)
    # to'g'ridan-to'g'ri get_value 1054 SQL xato berardi — himoya bilan.
    allow_multi_currency = False
    if item.get("pos_profile") and frappe.db.has_column("POS Profile", "posa_allow_multi_currency"):
...
def _get_scale_barcode_settings():
    ...
    try:
        ...
        frappe.log_error("Unable to load Scale Barcode Settings", "Suviner POS")
        return None
...
@frappe.whitelist()
def get_item_stock_by_warehouse(item_code, company=None):
    """Bitta tovarning barcha (guruh bo'lmagan, faol) omborlardagi qoldig'i.

    POS "Omborlar" oynasi uchun: Bin'da yozuvi bor omborlargina qaytadi
    (tovar harakat qilgan joylar). Bitta so'rov — tez.
    """
    if not item_code:
        return {"item_code": "", "stock_uom": "", "warehouses": []}
    ...
    query = (
        ...
        .inner_join(wh_dt)
        .on(bin_dt.warehouse == wh_dt.name)
        .select(
            bin_dt.warehouse,
            bin_dt.actual_qty,
            bin_dt.reserved_qty,
            bin_dt.projected_qty,
        )
        .where(bin_dt.item_code == item_code)
        .where(wh_dt.is_group == 0)
        .where(wh_dt.disabled == 0)
        .orderby(bin_dt.actual_qty, order=frappe.qb.desc)
    )
    if company:
        query = query.where(wh_dt.company == company)
...
def get_warehouses(company=None):
    """Guruh bo'lmagan, faol omborlar ro'yxati (ombor-ko'rish oynasi uchun)."""
...
@frappe.whitelist()
def get_warehouse_stock(warehouse, search=None, limit=500):
    """Bitta ombordagi barcha tovarlar qoldig'i (ixtiyoriy qidiruv bilan)."""
    if not warehouse:
        return {"warehouse": "", "items": [], "total": 0}
    ...
    query = (
        frappe.qb.from_(bin_dt)
        .inner_join(item_dt)
        .on(bin_dt.item_code == item_dt.name)
        .select(
            bin_dt.item_code,
            item_dt.item_name,
            bin_dt.actual_qty,
        ...
        .where(bin_dt.warehouse == warehouse)
        .where(item_dt.disabled == 0)
        .orderby(bin_dt.actual_qty, order=frappe.qb.desc)
    ...
    if term:
        like = f"%{term}%"
        query = query.where(
            (bin_dt.item_code.like(like)) | (item_dt.item_name.like(like))
        )
